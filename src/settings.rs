...
use lightyear::prelude::client::{Authentication, SocketConfig, SteamConfig};
...
pub(crate) fn get_server_net_configs(settings: &Settings) -> Vec<server::NetConfig> {
    settings
        ...
        .map(|t| match t {
            ...
            t => panic!("No server configuration for transport {t:?}")
        })
        .collect()
}
...
pub fn get_client_net_config(settings: &Settings, client_id: u64) -> client::NetConfig {
    ...
    match &settings.client.transport {
        ...
        t => panic!("No configuration for client transport {t:?}")
    }
}